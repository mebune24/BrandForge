...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification(err instanceof Error ? err.message : 'Login failed', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md mx-auto px-6">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-dark-blue-primary">Welcome Back</h2>
            <p className="text-gray-600 mt-2">Sign in to your BrandForge account</p>
          </div>
          {error && (
          ...
          <p className="text-center text-gray-600 text-sm mt-6">
            Don't have an account? <Link to="/register" className="text-blue-accent font-semibold hover:underline">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
